...
//#ifdef __linux__
//    if ( usb_set_configuration( mHandleFromPeriph.mDevHandle, 1 ) < 0 )
//    {
...
//#endif
    ...
    int actual_length = usb_bulk_read( mHandleFromPeriph.mDevHandle, mHandleFromPeriph.mEndpoint, p_data_from_device, rqstdLength, timeoutMs*30 );
    ...
    )
{
...
#ifdef __linux__
    // Linux libusb-0.1.12 wants this each time before claiming the
    // interface prior to a write operation (but not for a read operation)
    if ( usb_set_configuration( mHandleToPeriph.mDevHandle, 1 ) < 0 )
    {
    ...
    int actual_length = usb_bulk_write( mHandleToPeriph.mDevHandle, mHandleToPeriph.mEndpoint, p_data_to_device, size, timeoutMs*30 );
